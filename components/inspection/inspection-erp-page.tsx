...
export function InspectionErpPage({ module }: { module: RestaurantModuleDefinition }) {
  ...
  return (
    <ErpShell activeHref="/inspection">
      <div className="space-y-6">
        <ErpPageHeader
          breadcrumbs={["ME", "Store Operations", module.label[locale]]}
          title={preview.title}
        ...
        {preview.filters?.length ? (
          <MeWorkspaceSection
            title={locale === "zh" ? "工作区筛选" : "Workspace Filters"}
            description={locale === "zh" ? "当前巡检页面的运营筛选范围。" : "Operating filters for the inspection workspace."}
          >
        ...
        <MeDetailWorkspace
          main={
            <>
              {selectedRow ? (
                <MeWorkspaceSection
                  title={locale === "zh" ? "已选记录" : "Selected record"}
                  description={locale === "zh" ? "当前巡检记录的重点字段。" : "Key fields for the current inspection record."}
                >
