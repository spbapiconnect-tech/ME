import { toSupplierDisplayRecords, toSupplierIssueDisplayRecords } from "@/lib/display-adapters/psi";
import type { DataMeta } from "@/lib/data";
import { getPsiSupplierDetail, getPsiSupplierPageData } from "@/lib/services/psi";
...
export async function getPsiSupplierDetailPageData(id: string): Promise<PsiSupplierDetailPageData> {
  ...
  if (!result.ok || !result.data) {
    return { supplier: null, detailRows: [], meta: result.meta, isMock: result.meta.source === "mock", error: result.ok ? "Record not found" : result.error.message };
  }
